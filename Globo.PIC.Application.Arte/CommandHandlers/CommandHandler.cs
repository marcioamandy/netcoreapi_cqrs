...
namespace Globo.PIC.Application.Services.Commands
{
		...
		//IRequestHandler<CheckStatusChanged>
	{
		...
		{
			var trackingList = trackingRepository.GetAll().Where(s => s.IdPedidoItemArte == request.PedidoItemArteTracking.IdPedidoItemArte).ToList();

			//Verifica se o status solicitado já é o status atual
			if (trackingList.Count() > 0 && trackingList.Last().StatusId == request.PedidoItemArteTracking.StatusId)
				return Task.FromResult(Unit.Value); 

			var tracking = new PedidoItemArteTracking()
			{
				StatusId = request.PedidoItemArteTracking.StatusId,
				IdPedidoItemArte = request.PedidoItemArteTracking.IdPedidoItemArte,
        ...
        Task<Unit> IRequestHandler<DeleteTrackingArte, Unit>.Handle(DeleteTrackingArte request, CancellationToken cancellationToken)
        {
            var existTracking = trackingRepository.GetAll()
                .Where(i => i.IdPedidoItemArte == request.PedidoItemArteTracking.IdPedidoItemArte &&
                        i.StatusId == request.PedidoItemArteTracking.StatusId).ToList();
