...
{
    public class OrganizationStructureServiceProxy : IOrganizationStructureProxy
    {
        ...
        public OrganizationStructureServiceProxy(IHttpClientFactory _httpClientFactory,
        ...
        public async Task<List<OrganizationStructure>> GetOrganizationStructures(long businessUnityId,CancellationToken cancellationToken)
        {
            using (var client = httpClientFactory.CreateClient(DL_SERVICE_API))
            {
                try
                {
                    ...
                    string address = string.Format("emgt/proc-log/whse-mgmt/v1/projects/organizationStructure/{0}", businessUnityId.ToString());
                    ...
                    if (response.StatusCode == HttpStatusCode.OK)
                    {
                        ...
                        var lista = JsonConvert.DeserializeObject<OrganizationStructureItems>(res);
                        return lista.items;
                    }
                    else
                    {
